...
// using local_biz_types from local-bix-types.js
...
var app = {
  ...
  init : function (weekdays,categories) {
    ...
    dom.select_weekday.addEventListener( 'change', function () {
      ...
      if (app.focus_click && this.value != '(select)') {
  ...
  submit_form : function () {
    ...
    var category_field = document.querySelector('#select-type')
    ...
    schema_obj["@type"] = category_field.value
  ...
  'info' : {
    ...
    populate_options : function (options,parent_id) {
      ...
      var options = ['(select)'].concat(options)
      parent.innerHTML = ''
...
app.init(data.open_weekdays,data.categories)
